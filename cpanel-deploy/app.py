...
Struktur ini dirancang untuk berjalan di cPanel dengan Passenger WSGI.
File ini TIDAK menggunakan app.run() karena Passenger yang menangani server.

Setup cPanel:
1. Buat Python App di cPanel -> Setup Python App
2. Set Application root ke folder ini
3. Set Application startup file: passenger_wsgi.py
4. Set Application Entry point: application
...
# ── CPANEL PATH SETUP ────────────────────────────────────────────────────────
# Passenger di cPanel perlu INTERP check agar tidak reload terus-menerus
...
# ── DB PATH — Relative ke direktori aplikasi (cPanel compatible) ─────────────
# Di cPanel, gunakan path relatif bukan /tmp/ yang bisa di-clear
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB = os.path.join(BASE_DIR, "data", "vulnshop.db")
...
# ── INITIALIZE DB ON IMPORT (cPanel Passenger compatible) ────────────────────
init_db()
    ...
    <div class="container">
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body">
          ...
          <table class="table">
            <tbody>
              <tr><td>DB_PATH</td><td style="font-family:monospace">{DB}</td></tr>
...
def static_products(filename):
    return send_from_directory(os.path.join(BASE_DIR, "data"), "vulnshop.db")
...
# ══════════════════════════════════════════════════════════════════════════════
# CPANEL PASSENGER WSGI — Variabel `application` WAJIB ada untuk cPanel
# ══════════════════════════════════════════════════════════════════════════════
application = app


# ── LOCAL RUN (opsional, untuk testing lokal saja) ────────────
